...
from deepagents_talon.model_cache import model_cache_dir, validate_model_cache
...
def _load_local_pipeline(model: str, device: str, config: TalonConfig) -> _LocalSpeechPipeline:
    with _local_model_lock:
        return _initialize_local_pipeline(model, device, model_cache_dir(config))


def _initialize_local_pipeline(model: str, device: str, cache: Path) -> _LocalSpeechPipeline:
    ...
    snapshot = Path(hub.snapshot_download(repo_id=model, cache_dir=str(cache), token=False))
    validate_model_cache(cache)
    if not snapshot.resolve().is_relative_to(cache):
        msg = "Downloaded model snapshot must remain inside the Talon model cache"
